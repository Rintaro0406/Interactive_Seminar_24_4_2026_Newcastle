            ...
            with st.spinner("Running GLASS pipeline (this may take a while)..."):
                try:
                    ...
                    try:
                        ...
                        ax0.set_ylabel(r'$C_{\ell}$', fontsize=16)
                        ax0.tick_params(axis='both', which='major', labelsize=14)
                        ax0.grid(True, ls=':')

                        ax1 = axes[1]
                        ax1.semilogx(theta_deg, xi_plus, '-k')
                        ax1.set_xlabel('Angular separation [deg]', fontsize=16)
                        ax1.set_ylabel(r'$\xi_{+}(\theta)$', fontsize=16)
                        ax1.tick_params(axis='both', which='major', labelsize=14)
                        ax1.grid(True, ls=':')

                        ax2 = axes[2]
                        ax2.semilogx(theta_deg, xi_minus, '-k')
                        ax2.set_xlabel('Angular separation [deg]', fontsize=16)
                        ax2.set_ylabel(r'$\xi_{-}(\theta)$', fontsize=16)
                ...
                The calculations in this section are based on the **ΛCDM model**, the current standard model of cosmology.  
                We compute theoretical predictions using the **[GLASS](https://glass.readthedocs.io/stable/index.html)**, **[CAMB](https://camb.readthedocs.io/en/latest/)**, and **[HEALPix](https://healpix.sourceforge.io/)**, a widely used tool in modern cosmology.
